...
describe('UserService', () => {
  ...
  describe('deleteUserByUsername', () => {
    test('should delete a user when username exists', async () => {
      ...
      expect(userRepository.delete).toHaveBeenCalledWith({ username: username });
    });
    ...
    test('should throw an error when delete operation fails', async () => {
      ...
      (userRepository.delete as jest.Mock).mockRejectedValue(new Error('Delete operation failed'));

      await expect(userService.deleteUserByUsername(username)).rejects.toThrow('Delete operation failed');
      expect(userRepository.delete).toHaveBeenCalledWith({ username: username });
    });
  });
  ...
  describe('getUserByUsername', () => {
    ...
    test('should throw an error when the repository operation fails', async () => {
      ...
      (userRepository.findOne as jest.Mock).mockRejectedValue(new Error(errorMessage));
      await expect(userService.getUserByUsername(username)).rejects.toThrow(errorMessage);
      expect(userRepository.findOne).toHaveBeenCalledWith({ where: { username } });
    });
  });
  ...
  describe('createUser', () => {
    ...
    test('should throw an error when the repository insert operation fails', async () => {
      const user = { id:1,
        ...
        userStatus: 0 }; // 示例用户对象
      const errorMessage = 'Database Error';
      (userRepository.insert as jest.Mock).mockRejectedValue(new Error(errorMessage));
  ...
  describe('updateUserInfo',() => {
    ...
    test('should update user info when user exists', async () => {
      ...
      jest.spyOn(userService, 'getUserByUsername').mockResolvedValue(updateUserDto);
      // 模拟仓库的 update 方法
    ...
    test ('should throw NotFoundException when user does not exist', async () => {
      ...
      await expect(userService.updateUserInfo(username, updateUserDto))
